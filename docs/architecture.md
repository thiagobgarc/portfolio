...
build's page list; `public/robots.txt` points at it. The default OG image
(`public/images/og-default.png`) is a build-time-rasterized PNG rather than SVG, since several
social crawlers (Twitter/X in particular) don't render SVG `og:image` tags. Per-project OG images
are a worthwhile follow-up, not yet built.
